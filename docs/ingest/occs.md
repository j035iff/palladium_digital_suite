...
- `occRelatedSkills` — `initialSlotsCount`, `categoryRules` (and optional `skillVouchers`, legacy `categoryMinimums`, `startingSkillIds`)

`skillVouchers` encode cluster/OR/fixed specialization picks in creation UI. When a voucher lists a category whose `categoryRules` row is `accessType: "none"`, the engine **opens that category for voucher picks only** (book pattern: “None, except as special vocational training, above”). General related browsing still respects `none`.
...
| Mind Master P.C.C. — bonus psionics | 2 Super OR 3 Sensitive at 1st level | **Player completes one `creationSelectionPlan` branch only** — same UX as Psychic Gate Major allocation (8-from-one vs 6-mixed); both branches stay in plan with exclusive labels |
| Psi-Mechanic P.C.C. — per-level device powers | 2 minor-category OR 1 Super per level | **Same exclusive-branch UX as Mind Master ruling-01** — branch A in `perLevelSelection`, branch B in `creationSelectionPlan`; player completes one branch per level |
| BTS 'Kinetic P.C.C. Pass B | pp. 120–122 major power + lore | Deep **`classAbilities`** on base row (I.S.P., persecution/Spook Squad lore) + per-specialization major-power modifiers (electro arcs, hydro jets, pyro fireball discount, TK lift/range); `specializations[]` + `ispEngine` from Pass A |
...
| General Citizen (`occ_general_citizen`) | Survival Guide Education Modifiers pp. 61–65 | **One O.C.C. row + 12 `specializations[]`** (D100 bands in description prose). **Not** `nightbane_rcc` — `occType: civilian`; no `raceRestrictions`; available to any race with `canPickOcc: true`. Nightbane adds to `allowedOccIds` / `allowedOccPackageIds`. Specializations **replace** `occSkillsCore`, `secondarySkills`, `levelUpSkillChoices`; `occRelatedSkills.replaceBaseline: true` for full category programs. **XP:** `progression.xpTableSource: race` — Nightbane → `nightbane_core_nightbane_guardian`; Human → `nightbane_core_doppleganger` (core p. 233). Regenerate catalog: `python scripts/generate-occ-general-citizen.py` |
| General Citizen — Street Schooled Pass B | WB5 pp. 61–62 street survivor | Deep **`classAbilities`** for no formal education, street survival lifestyle, young/NSB background, included Facade H2H Basic, civilian W.P., Technical Lore +10% rule, related/secondary advancement, Education Modifiers option, Morphus combat cross-ref; **`startingEquipment`** for impoverished street kit; **`finances`** `2D6×10` (book silent — below core civilian cash); Pass A skill package already encoded |
| General Citizen — Street Thug Pass B | WB5 pp. 61–62 small-time criminal | Deep **`classAbilities`** for no formal education, wrong-crowd/street-muscle background, included Facade H2H Expert, street W.P. (Auto Pistol core; no pole/whip/heavy ban only), related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for petty-criminal street gear; **`finances`** `2D6×100` (book silent); Pass A skill package already encoded |
| General Citizen — High School Educated Pass B | WB5 pp. 61–62 high-school graduate | Deep **`classAbilities`** for graduate background, young-adult demographic, included Facade H2H Basic, civilian W.P., computer-literate core, limited Rogue access, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for educated civilian baseline; **`finances`** `3D6×100` (core civilian parity); Pass A skill package already encoded |
| General Citizen — Associate's / Vocational Pass B | WB5 pp. 62–63 two-year trade education | Deep **`classAbilities`** for vocational background, +10% cluster rule (Communications/Electrical/Medical/Mechanical/Science), Domestic/Pilot minimum, expanded related access, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for trade-school gear + tools; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — College Graduate Pass B | WB5 p. 63 bachelor's degree | Deep **`classAbilities`** for university background, +20% cluster rule (adds Technical), scholastic core skills, broad related access, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for academic/professional gear; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Rescue / First Responder Pass B | WB5 pp. 63–64 fire/EMS/law | Deep **`classAbilities`** for occupational background, included Facade H2H Basic (Expert×1/MA×2), +20% Medical/Technical specialty, Pilot minimum, law-enforcement Espionage/W.P. exceptions, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for service/rescue gear; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Laborer / Worker: Light Pass B | WB5 p. 64 semi-skilled laborer | Deep **`classAbilities`** for worker background, +15% work-cluster rule, practical core skills, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for work clothes/tools/vehicle; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Heavy Laborer / Construction Pass B | WB5 p. 64 skilled tradesworker | Deep **`classAbilities`** for construction/trade background, +20% specialty cluster (Electrical/Mechanical/Pilot/Technical), demolition Military access, construction W.P. rules, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for job-site gear; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Mechanic & Repair Pass B | WB5 pp. 64–65 skilled mechanic | Deep **`classAbilities`** for mechanic background, four Mechanical specialty skills (+15%), mechanic core skills, limited Rogue access, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for shop/tools/vehicle; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Computer / Electronics Pass B | WB5 p. 65 tech specialist | Deep **`classAbilities`** for computer/electronics background, digital core skills, Communications/Technical +15% minimums, Technical Lore +10% rule, no Rogue/Wilderness related access, related/secondary advancement, Morphus combat cross-ref; **`startingEquipment`** for PC/networking/electronics bench; **`finances`** `3D6×100`; Pass A skill package already encoded |
| General Citizen — Military / Soldier Pass B | WB5 p. 65 ex-military | Deep **`classAbilities`** for service background, included Facade H2H Basic (Expert×1/MA×2), any W.P., Military/Physical/Comm-Espionage minimums (+20% note), broad military related access, Morphus combat cross-ref; **`startingEquipment`** for service weapons/gear; **`finances`** `3D6×100`; Pass A skill package already encoded |
| O.C.C. core voucher per-skill bonus | Pick-one with different % per option (e.g. Bicycling +16% or Automobile +4%) | **`skillSpecificOverrides` on `occCoreSkillChoiceVoucher`** — overrides `bonusPercent` for listed skill ids; engine resolves in creation display and spawn handoff |
