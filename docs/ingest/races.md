...
| Snake Bird (gm_approval) | Natural spell caster R.C.C. | **`race_snake_bird`** + **`occ_snake_bird_rcc`**; shares **`nightbane_core_snakebird_mystic`** XP table with Mystic O.C.C.; P.P.E./spell roadmap on shadow O.C.C.; `psionics.capabilityType: none`. |
