...
export function PendingDiceResolutionPanel({
  ...
  const diceComplete = pendingDiceBlocksResolutionComplete(blocks, resolutions)
    ...
    return (
      <div className="space-y-2 text-xs">
        ...
        <p className="pt-1 text-[10px] text-slate-700 dark:text-slate-200" role="status">
          {entered}/{totalRolls} rolls entered
  ...
  return (
    <div className="rounded-lg border-2 border-blue-400 bg-white p-4 shadow-sm dark:border-blue-600 dark:bg-slate-950">
      ...
      <p className="mb-4 text-xs leading-snug text-slate-800 dark:text-slate-200">
        ...
        start from the ledger flat value and update as you enter results (Pillar 5 — physical
        dice first).
      </p>
      ...
      {preview.length > 0 ? (
        <div className="mt-4 rounded-md border border-slate-300 bg-slate-100 p-3 dark:border-slate-600 dark:bg-slate-900">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-wide text-slate-800 dark:text-slate-100">
            Vitality preview
          </p>
          <div className="grid gap-1 font-mono text-xs text-slate-900 dark:text-slate-100 sm:grid-cols-2">
            {preview.map((line) => (
              <p key={line.label}>
